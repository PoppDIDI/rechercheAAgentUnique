...
def dessiner_grille(ecran, grille, compteur, k, selectionnee=None):
    ...
    ecran.blit(texte_instruction, (10, 60))
...
def entrer_k(ecran):
    ...
    while actif:
        ecran.fill(BLANC)
        texte_titre = font.render("Entrez le nombre de déplacements (k):", True, NOIR)
...
# Demander le nombre initial de déplacements (k)
k = entrer_k(ecran)
compteur_deplacement = 0
swap_effectue = False  # Indicateur pour savoir si le swap a été fait
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if compteur_deplacement < k or (compteur_deplacement == 0 and swap_effectue):
                # Déplacement avec clic souris sur une tuile adjacente au trou
                ...
                # Swap autorisé uniquement lorsque compteur == k
                if selectionnee is None:
                    ...
                    selectionnee = None
                    swap_effectue = True  # Swap effectué
                    compteur_deplacement = 0  # Réinitialiser le compteur
                    swap_effectue = False  # Permettre un nouveau cycle de déplacements
