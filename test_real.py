...
def dessiner_grille(ecran, grille, taille, compteur, k, selectionnee=None):
    ...
    OFFSET_X = (TAILLE_ECRAN - (TAILLE_TUILE * taille)) // 2
    OFFSET_Y = (TAILLE_ECRAN - (TAILLE_TUILE * taille)) // 2 + 50

    ecran.fill(BLANC)
    font = pygame.font.Font(None, 50)

    # Dessiner les informations au-dessus des cases
    texte_deplacement = font.render(f"Déplacements: {compteur}/{k}", True, NOIR)
    texte_instruction = font.render(
        "Swap possible !" if compteur == k else "Déplacez les tuiles !", True, ROUGE if compteur == k else NOIR
    )
    ecran.blit(texte_deplacement, (50, 10))
    ecran.blit(texte_instruction, (50, 50))

    # Dessiner les cases centrées
    for y, ligne in enumerate(grille):
        for x, tuile in enumerate(ligne):
            rect = pygame.Rect(
                OFFSET_X + x * TAILLE_TUILE, OFFSET_Y + y * TAILLE_TUILE, TAILLE_TUILE, TAILLE_TUILE
            )
            if (x, y) == selectionnee:
...
def position_souris_en_grille(pos, taille):
    ...
    OFFSET_Y = (TAILLE_ECRAN - (TAILLE_TUILE * taille)) // 2 + 50
    x, y = pos
    return (x - OFFSET_X) // TAILLE_TUILE, (y - OFFSET_Y) // TAILLE_TUILE

# Fonction pour afficher l'interface de configuration
def afficher_interface(ecran):
    ...
    while actif:
        ecran.fill(BLANC)
        texte_titre = font.render("Configuration du jeu", True, NOIR)
